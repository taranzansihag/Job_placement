	...
	public void actionPerformed(ActionEvent e){
	   ...
	   if(e.getSource()==jb){
			   try{
				...
				ResultSet rs = st.executeQuery("select * from company where cid = '"+s+"' ");
				while(rs.next()){
				  flag=1;
				  break;
				}
		...
		if(flag==1){
		st.execute("delete from company where cid = '"+s+"' ");
		 Company c = new Company("delete panel");
		 c.setSize(700,700);
		 c.setVisible(true);
