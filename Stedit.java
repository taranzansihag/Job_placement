...
public class Stedit extends JFrame implements ActionListener{
  ...
  public Stedit(String s,String p){
	super(s);
  ...
  public void actionPerformed(ActionEvent e){
	String p =jt.getText();
	 ...
	 try{
		...
		ResultSet rs = st.executeQuery("select * from student where sid = '"+p+"' ");
		...
		if(e.getSource()==jb){
		   st.execute("update student set emailid='"+email+"' , fname='"+fnm+"' , lname='"+lnm+"' , address='"+add+"' , mob='"+mob+"' , gender='"+g+"' , 10th='"+ten+"' , 12th='"+twlth+"' , branch='"+branch+"' , back='"+bk+"' , no_of_back='"+nbk+"' , country='"+country+"' where sid='"+p+"' ");
