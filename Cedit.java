...
import javax.swing.*;
import java.util.*;
public class Cedit extends JFrame implements ActionListener{
	JLabel jl;
    JTextField jt;
    JButton jb,jb1;
    public Cedit (String s){
	super(s);
	setLayout(null);
	jl = new JLabel("cid");
	...
	public void actionPerformed(ActionEvent e){
	   ...
	   if( e.getSource()==jb){
			   try{
				...
				ResultSet rs = st.executeQuery("select * from company where cid = '"+cid+"' ");
				while(rs.next()){
				  flag=1;
				  break;
				}
	     if(flag==1){
		Cpedit std= new Cpedit(cid);
		 std.setSize(700,700);
		 std.setVisible(true);
